...
class SimuladorDeImoveisViewTest(TestCase):
    ...
    def test_get_resultado_da_simulacao_com_dados_invalidos_retorna_para_formulario(
        self,
    ):
        self.validSimulationData["valor_da_entrada"] = 1000
        response = self.client.get(
            reverse("imoveis:resultado-simulacao"), self.validSimulationData
        )
        ...
        self.assertEqual(
            dados_iniciais["data_nascimento"].strftime("%Y-%m-%d"), "1997-08-09"
        )
    ...
    def test_resposta_da_simulacao_contem_objeto_simulacao_e_tabela(self):
        ...
        self.assertEqual(simulacao.amortizacao, "PRICE")
        self.assertEqual(
            simulacao.valor_do_imovel, self.validSimulationData["valor_do_imovel"]
        )
        self.assertEqual(
            simulacao.valor_da_entrada, self.validSimulationData["valor_da_entrada"]
        )
        self.assertEqual(
            int(simulacao.prestacoes), self.validSimulationData["prestacoes"]
        )
    ...
    def test_resposta_da_simulacao_SAC_contem_objeto_simulacao_e_tabela(self):
        self.validSimulationData["amortizacao"] = "SAC"
        response = self.client.get(
            reverse("imoveis:resultado-simulacao"), self.validSimulationData
        )
        self.assertEqual(response.status_code, 200)
        ...
        simulacao = response.context["simulacao"]
        # testa se valores da simulação estão corretos
        self.assertEqual(simulacao.amortizacao, "SAC")
        self.assertEqual(
            simulacao.valor_do_imovel, self.validSimulationData["valor_do_imovel"]
        )
        self.assertEqual(
